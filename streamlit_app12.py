...
def chat(prompt):
    ...
    student, teacher = st.session_state[f"student_agent_{profile}"], st.session_state[f"teacher_agent_{profile}"]
    ...
    for m in result.chat_history:
        name, content = m.get("name", ""), m.get("content", "").strip()
        if name == "Student_Agent" and (not student_said_prompt or content != prompt.strip()):
            st.session_state[key].append({"role": "student", "content": content})
