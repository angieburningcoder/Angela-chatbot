...
def init_session_state():
    defaults = {
        ...
        "profile_list": ["KA助理", "職歷顯細", "日常聊天", "Interdisciplinary Study Planner"],
        "chat_stage": "init",
        "json_response": None
    }
...
def init_agent(profile, lang, model):
    ...
    if profile == "Interdisciplinary Study Planner":
        system_prompt = f"""
...
def render_study_plan(data):
    result = """
### 🧭 主修 ➜ 目標
- **主修 (Current Major):** {current_major}
- **目標 (Desired Field):** {desired_field}
...
""".format(
        ...
        courses="\n".join([f"- **{c['course_name']}**：{c['description']}" for c in data.get("recommended_courses", [])]),
        projects="\n".join([f"- **{p['project_name']}**：{p['description']}" for p in data.get("possible_projects", [])]),
        interns="\n".join([f"- {i}" for i in data.get("suitable_internship_directions", [])]),
        networking="\n".join([f"- {n}" for n in data.get("networking_strategies", [])]),
        timeline="\n".join([f"- **{y}**：{t}" for y, t in data.get("timeline_advice", {}).items()])
    )
    return result
...
def render_message(msg):
    ...
    st.markdown(
        f"<div style='text-align: left; background-color: {color}; padding: 8px; border-radius: 10px; margin: 4px 0;'>{icon} {content}</div>",
...
def main():
    ...
    if prompt := st.chat_input(placeholder="💡請描述你的主修與想轉職／跨領域的目標，例如：「我主修心理學，想做產品經理，有哪些選課與人脈建議？」"):
        chat(prompt)
